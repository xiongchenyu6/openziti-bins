...
in with pkgs; rec {
  ...
  srcBinZiti = version: sha256:
    fetchurl {
      inherit sha256;
      url =
        "https://github.com/openziti/ziti/releases/download/v${version}/ziti-linux-amd64-${version}.tar.gz";
    };
  ...
  srcBinZitiEdgeTunnel = version: sha256: {
    x86_64-linux = fetchzip {
      inherit sha256;
      url =
        "https://github.com/openziti/ziti-tunnel-sdk-c/releases/download/v${version}/ziti-edge-tunnel-Linux_x86_64.zip";
    };

    x86_64-darwin = fetchzip {
      inherit sha256;
      url =
        "https://github.com/openziti/ziti-tunnel-sdk-c/releases/download/v${version}/ziti-edge-tunnel-Darwin_x86_64.zip";
    };

    aarch64-darwin = fetchzip {
      inherit sha256;
      url =
        "https://github.com/openziti/ziti-tunnel-sdk-c/releases/download/v${version}/ziti-edge-tunnel-Darwin_arm64.zip";
    };
  };
    ...
    lib.optionalAttrs (system == "x86_64-linux") {
      "ziti-cli-functions_${v}" = writeShellApplication {
        ...
        text = let
          ...
          zitiCliFnSrc = (srcZiti version hash)
  ...
  mkZitiConsole = inputs': self:
    lib.optionalAttrs (system == "x86_64-linux") {
      ziti-console = let
        napalmPackage = inputs'.napalm.legacyPackages.buildPackage
          self.inputs.zitiConsole.outPath {
            npmCommands =
              "npm install --no-audit --loglevel verbose --ignore-scripts --nodedir=${nodejs}/include/node";
          };
  ...
  mkZitiEdgeTunnelPkg = v: state: system: {
    "ziti-edge-tunnel_${v}" = stdenv.mkDerivation rec {
      ...
      src = (srcBinZitiEdgeTunnel version
        state.srcBinZitiEdgeTunnel.${system}.${v}.hash).${system};
      sourceRoot = ".";
      nativeBuildInputs =
        lib.optionals (system == "x86_64-linux") [ autoPatchelfHook ];
      runtimeDependencies =
        lib.optionals (system == "x86_64-linux") [ systemd ];
      ...
      meta = {
        ...
        description =
          "Ziti: programmable network overlay and associated edge components for application-embedded, zero-trust networking";
        license = licenses.asl20;
  ...
  mkZitiPkgs = state:
    foldl (acc: v: acc // (mkZitiPkg v state)) { } (attrNames state.srcBinZiti);
  mkZitiBinTypePkgs = state: binType:
    foldl (acc: v: acc // (mkZitiBinTypePkg v binType state)) { }
    (attrNames state.srcZiti);
  mkZitiCliFnPkgs = state:
    foldl (acc: v: acc // (mkZitiCliFnPkg v state)) { }
    (attrNames state.srcZiti);
  mkZitiEdgeTunnelPkgs = state: system:
    foldl (acc: v: acc // (mkZitiEdgeTunnelPkg v state system)) { }
    (attrNames state.srcBinZitiEdgeTunnel.${system});
}
